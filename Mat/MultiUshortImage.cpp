...
bool MultiUshortImage::Extend2Image(MultiUshortImage *pInImage, MultiUshortImage *pOutImage,int nS)//2��x�η�
{
...
bool MultiUshortImage::Load16BitRawDataFromBinFile(char *pFileName, int nWidth, int nHeight, int nBits, bool bHighBit, bool bByteOrder, int nMIPIRAW)//bByteOrderС����������С��ģʽ��С���ֽ�����������Ч�ֽ����ڵ͵�ַ�ϵ��ֽڴ�ŷ�ʽ��bHighBit �ߵ�λ��Ч nMIPIRAW==0��ʾ����mipi ������ʾ��ͬmipi
{
...
{
	unsigned int *ColSum = (unsigned int *)calloc(Width*dim, sizeof(unsigned int));        //    �õ�calloc����Ŷ���Զ��ڴ���0
	memset(Integral->GetImageData(), 0, (Width + 1)*dim * sizeof(unsigned int));
	for (int y = 0; y < Height; y++)
	{
		unsigned short *LinePS = GetImageLine(y);// pInData + y * Stride;
		unsigned int *LinePD = Integral->GetImageLine(y + 1) + dim;// +(y + 1) * (Width + 1) + 1;//����ͼ1 1��ʼһ����ʼ
		for (int k=0;k<dim;k++)
		{
			LinePD[-dim+k] = 0;
		}
		for (int x = 0; x < Width; x++)//�з���Ļ����ۼ�
		{
			for (int k=0;k<dim;k++)
			{
				ColSum[x*dim+k] += LinePS[x*dim+k];//ColSumͳ�Ƶ��ǵ��ۼ�LinePSͳ���е��ۼ�
