...
extern void HistIntToHistfloat(int pHist[256], float pfHist[256])
{
	for (int i = 1; i < 256; i++)
	{
		pHist[i] += pHist[i - 1];//�ۼ�pHist�����������֮ǰ���е��ܺ�
	}
	//pHist[255]����������=nx*ny;
	for (int i = 0; i < 256; i++)
...
extern void FindHistSpecification(float SrcHistNum[256], float DstHistNum[256], int map[256])
{
	...
	for (int y = 0; y < 256; y++)
	{
		for (int x = 0; x < 256; x++)
		{
			srcMin[x][y] = fabs(SrcHistNum[y] - DstHistNum[x]);//srcMin[x][y]��¼ԭֱ��ͼ��y��������
		}
	}
	for (int x = 0; x < 256; x++)//һά�ҵ���С
	{
		minValue = srcMin[x][0];
		for (int y = 0; y < 256; y++)//�Ӷ�ά�ҵ���С��
		{
...
extern void VGaussianLine3(unsigned short *pInLines[3], unsigned char *pOutLine, int nWidth,int nChannel)//����洢
{
	int  x;
	x = 0;
#ifdef USE_NEON
...
{
	unsigned int *ColSum = (unsigned int *)calloc(Width, sizeof(unsigned int));        //    �õ�calloc����Ŷ���Զ��ڴ���0
	memset(Integral, 0, (Width + 1) * sizeof(unsigned int));
	for (int Y = 0; Y < Height; Y++)
	{
		unsigned char *LinePS = pInData + Y * Stride;
		unsigned int *LinePD = Integral + (Y + 1) * (Width + 1) + 1;//����ͼ1 1��ʼһ����ʼ
		LinePD[-1] = 0;
		for (int X = 0; X < Width; X++)//�з���Ļ����ۼ�
		{
			ColSum[X] += LinePS[X];//ColSumͳ�Ƶ��ǵ��ۼ�LinePSͳ���е��ۼ�
...
extern inline void ftofHSmoothLinex7(float *pInLine, float *pOutLine, int nWidth)//
{
	...
	for (x = 3; x < tmpnWidth; x += 4)
	{
		...
		pOutLine += 1;//+5\CA\C7?\C1\CB\C8\C3\CF\E0\C1?\C45\D0��\C4\CA\FD\BE?\E5\C8?\C6\E4\D6Т\AC\B8\F8\BA\F3\C3\E6\B5?\B9?\B7\BD\CF\F2?\C3\E8\D7\F6?\B1\B8\CF\E0\C1?\C45\D0\D0
