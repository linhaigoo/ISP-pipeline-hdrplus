	...
	for (int y = 0; y < nHeight; y += Step)
	{
		...
		for (int k = 0; k < nFrame; k++)
		{
			...
			for (int x = 0; x < nWidth; x += Step)
			{
				...
				if (k == 0)
				{
					weiget = (unsigned short)((float)SCALEVALUE / (*pTotalWeight++));//�Ŵ�2��14�η�
					Newy = y;
					Newx = x;
				}
				else
				{
					weiget = (unsigned short)((float)SCALEVALUE * (*pWeightline++) / (*pTotalWeight++));//�Ŵ�2��14�η�
...
bool CHDRPlus_BlockMatchFusion::UpScaleOffsetAndValuex2(MultiShortImage *pInImage, MultiShortImage *pOutImage)//С ��
{
	int nWidth = pInImage->GetImageWidth();
	int nHeight = pInImage->GetImageHeight();
	int tnx2 = nWidth * 2;
